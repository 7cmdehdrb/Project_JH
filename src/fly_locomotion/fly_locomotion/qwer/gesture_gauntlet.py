...
class IntegratedLocomotion(Node):
    ...
    def calculate_translation_vel(self):
        ...
        speed_mag = self._linear_velocity_mapping(delta_norm)
        ...
        lin_vel_vec = np.array(
            [
                speed_mag * (delta[0] / delta_norm) if delta_norm > 1e-6 else 0.0,
                speed_mag * (delta[1] / delta_norm) if delta_norm > 1e-6 else 0.0,
                speed_mag * (delta[2] / delta_norm) if delta_norm > 1e-6 else 0.0,
            ]
        )

        print(
            f"Delta Pos: {delta}, Speed Mag: {speed_mag}, Lin Vel Vec (Local): {lin_vel_vec}"
        )

        # rot90 = R.from_euler("z", -90, degrees=True).as_matrix()
        # lin_vel_vec = rot90 @ lin_vel_vec

        # print(f"Linear Vel Vec: {lin_vel_vec}")
    ...
    def calculate_rotation_vel(self):
        ...
        ang_vel_vec = ang_vel_mag * delta_axis

        # Kill roll
        delta_rot = R.from_rotvec(ang_vel_vec * self.dt)
        roll, pitch, yaw = delta_rot.as_euler("xyz", degrees=True)
        roll = 0.0
        delta_rot = R.from_euler("xyz", [roll, pitch, yaw], degrees=True)
        ang_vel_vec = delta_rot.as_rotvec() / self.dt
    ...
    def move_next_frame(self, lin_vel_vec, ang_vel_vec):
        ...
        # Pring velocity magnitude for debugging
        lin_vel_mag = np.linalg.norm(lin_vel_vec)
        ang_vel_mag = np.linalg.norm(ang_vel_vec)

        print(f"Linear Vel Mag: {lin_vel_mag:.4f}, Angular Vel Mag: {ang_vel_mag:.4f}")
