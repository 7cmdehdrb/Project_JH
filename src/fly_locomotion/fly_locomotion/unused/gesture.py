...
class Gesture(Node):
    def __init__(self):
        ...
        # if gpus:
        #     try:
        #         # 첫 번째 GPU만 사용하거나 필요한 만큼만 메모리를 점유하도록 설정
        #         for gpu in gpus:
        ...
        self.model = tf.keras.models.load_model(
            "/home/min/7cmdehdrb/wwg_ws/train_251224.h5"
        )
        ...
        self.pub = self.create_publisher(
            msg_type=String, topic="gesture", qos_profile=QoSProfile(depth=10)
        )
    ...
    def skeleton_callback(self, msg: PoseArray):
        ...
        window_size = 10
        if not hasattr(self, "prediction_window"):
            self.prediction_window = []
        if not hasattr(self, "last_prediction"):
            self.last_prediction = 1  # Initialize with 'Unknown' label

        # Add the current prediction to the sliding window
        self.prediction_window.append(np.argmax(prediction))
        if len(self.prediction_window) > window_size:
            self.prediction_window.pop(0)

        # Count the occurrences of the most frequent label in the window
        frequency = self.prediction_window.count(not self.last_prediction)

        if self.last_prediction == 0:
            if frequency >= 3:
                predicted_label = 1  # Switch to 'Unknown'
            else:
                predicted_label = 0  # Stay as 'Pointing'
        else:
            if frequency >= 1:
                predicted_label = 0  # Switch to 'Pointing'
            else:
                predicted_label = 1  # Stay as 'Unknown'

        self.last_prediction = predicted_label

        # Publish the label based on the averaged prediction
